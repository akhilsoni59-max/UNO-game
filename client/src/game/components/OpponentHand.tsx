...
export function OpponentHand({ player, seat, onCatch }: Props) {
  const count = player.cardCount;
  const show = Math.min(count, seat.maxVisibleCards);
  ...
  return (
    <div className={`gc-seat ${player.isTurn ? "is-turn" : ""} ${player.connected === false ? "is-offline" : ""}`}>
      {slots.map((s, i) => (
        <div
          key={i}
      ...
      >
        <div className={`gc-avatar ${player.isTurn ? "pulse" : ""}`}>
          <span>{initials(player.name)}</span>
          {player.isTurn && <div className="gc-turn-ring" />}
        </div>
